...
ready to receive/send data. This code connects this line to a GPIO interrupt which gives the ready semaphore. The main
...
static int      source_size, prev_size;


//The semaphore indicating the slave is ready to receive stuff.
static xQueueHandle ready;
...
static void IRAM_ATTR gpio_handshake_isr_handler(void* arg)
{
    ...
    xSemaphoreGiveFromISR(ready, &mustYield);
...
void app_main(void)
{
    ...
    for (size_t i = 0; i < 10; i++)
    {
        ...
        // }
        t.length    = sizeof(source_buffer) * 8;
        t.tx_buffer = source_buffer;
        t.rx_buffer = receive_buffer;

        ESP_LOG_BUFFER_HEX("Sent", source_buffer, source_size);

        //Wait for slave to be ready for next byte before sending
        xSemaphoreTake(ready, portMAX_DELAY);
        ret = spi_device_transmit(handle, &t);

        // ESP_LOG_BUFFER_HEX("Received", receive_buffer, prev_size);

        prev_size = source_size;
