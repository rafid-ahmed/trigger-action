...
void parseTriggerActionPayload( char *media, uint8_t priority, uint8_t *payload, uint8_t payloadLength, bool deprovision = false ) ;
...
//    ESP_LOGI( TAG, "onTriggerDefinition: id = %u", trigger->trigger_id );
    ...
    for ( int i = 0; i < trigger_driver_count; i++ ) {
        ...
        if ( ( trigger_driver->trigger_type == trigger->trigger_type )
            ...
            if ( deprovision ) {
                ESP_LOGI( TAG, "Removing trigger, id = %u", trigger->trigger_id );
                trigger_driver->driver->removeTrigger( trigger_driver->driver->state, trigger );
            } else {
                // Create a new trigger definition
                ESP_LOGI( TAG, "Adding a new trigger, id = %u", trigger->trigger_id );
...
//    ESP_LOGI( TAG, "onActionDefinition: id = %u", action->action_id );
    ...
    for ( int i = 0; i < action_driver_count; i++ ) {
        ...
        if ( ( actionDriver->action_type == action->action_type )
            ...
            if ( deprovision ) {
                ESP_LOGI( TAG, "Removing action, id = %u", action->action_id );
                actionDriver->driver->removeAction( actionDriver->driver->state, action );
            } else {
                // Create a new action definition
                ESP_LOGI( TAG, "Adding a new action, id = %u", action->action_id );
...
//    ESP_LOGI( TAG, "onRuleDefinition: id = %u", rule->rule_id );
    ...
    // Create a new rule
    ESP_LOGI( TAG, "Adding a new rule, id = %u, %u -> %u", rule->rule_id, rule->trigger_id, rule->action_id );
...
// Buzzer Driver ------------------------------------------------------------------------------------
    ...
    // adBytes = ProtoGen_NewButtonTrigger( 20, 1, 0, &adLength );
    // networkWrite( adBytes, adLength, PRIORITY_LOW, NON_PERSISTENT);
    // free( adBytes );
    // // Button Trigger: #1 Up
    // adBytes = ProtoGen_NewButtonTrigger( 21, 1, 1, &adLength );
    // networkWrite( adBytes, adLength, PRIORITY_LOW, NON_PERSISTENT);
    // free( adBytes );
    ...
    // // Rule: Down -> Blue
    // adBytes = ProtoGen_NewRule( 20, 20, 20, &adLength );
    // networkWrite( adBytes, adLength, PRIORITY_LOW, NON_PERSISTENT);
    // free( adBytes );
    // // Rule: Up -> Off
    // adBytes = ProtoGen_NewRule( 21, 21, 21, &adLength );
    // networkWrite( adBytes, adLength, PRIORITY_LOW, NON_PERSISTENT);
    // free( adBytes );
