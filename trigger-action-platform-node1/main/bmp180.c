...
esp_err_t bmp180_read_raw_temperature( i2c_port_t i2c_num, uint16_t *value ) {
    ...
    /* Need to wait 4.5 ms */
    // XXX: vTaskDelay works on ticks, by default a tick is 10ms
    //TickType_t delay_ticks = 5 / portTICK_RATE_MS;
    //ESP_LOGI( TAG, "Delaying %u ticks, portTICK_RATE_MS: %u", delay_ticks, portTICK_RATE_MS );
...
esp_err_t bmp180_read_raw_pressure( i2c_port_t i2c_num, bmp180_oversampling_t oversampling, uint32_t *value ) {
    ...
    ets_delay_us( bmp180_delays[ oversampling ] * 1000 );
...
esp_err_t bmp180_read_true_temperature( i2c_port_t const i2c_num, bmp180_eeprom_t const eeprom, int32_t *value ) {
    uint16_t raw_temperature;
    esp_err_t ret = bmp180_read_raw_temperature( i2c_num, &raw_temperature );
    if ( ret != ESP_OK ) return ret;

    *value = bmp180_true_temperature( raw_temperature, eeprom );
    return ESP_OK;
}

esp_err_t bmp180_read_true_temperature_pressure( i2c_port_t const i2c_num, bmp180_eeprom_t const eeprom,
                                    bmp180_oversampling_t const oversampling, int32_t *temperature,
                                    int64_t *pressure ) {
    uint32_t raw_pressure;
    esp_err_t ret = bmp180_read_raw_pressure( i2c_num, oversampling, &raw_pressure );
    if ( ret != ESP_OK ) return ret;
    
    uint16_t raw_temperature;
    ret = bmp180_read_raw_temperature( i2c_num, &raw_temperature );
    if ( ret != ESP_OK ) return ret;

    *temperature = bmp180_true_temperature( raw_temperature, eeprom );
    *pressure = bmp180_true_pressure( raw_pressure, raw_temperature, eeprom, oversampling );

//    ESP_LOGI( TAG, "Pressure = (raw: %u, comp: %lld), temperature = (raw: %u, comp: %d)", raw_pressure, *pressure,
