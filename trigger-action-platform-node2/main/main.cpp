...
uint8_t definition_count = 0;
#define TOTAL_DEFINITIONS 12
    ...
    for ( int i = 0; i < trigger_driver_count; i++ ) {
        ...
        if ( ( trigger_driver->trigger_type == trigger->trigger_type )
            ...
            if ( deprovision ) {
                ...
                definition_count++;
                if ( definition_count == TOTAL_DEFINITIONS ) {
...
// BMP180 Driver ------------------------------------------------------------------------------------
    ...
    TriggerDriver *bmp180driver = Bmp180dr_NewDriver( onLocalSignal, I2C_NUM_0 );
        ...
        trigger_drivers[ trigger_driver_count ] = (action_driver_s *)malloc( sizeof( trigger_driver_s ) );
...
// Button Driver ------------------------------------------------------------------------------------
	...
	TriggerDriver *joystickDriver = Joystick_NewDriver(onLocalSignal, 13, (adc1_channel_t )CONFIG_DRIVER_JOYSTICK_X_ADC_CHAN, (adc1_channel_t )CONFIG_DRIVER_JOYSTICK_Y_ADC_CHAN);
	trigger_drivers[ trigger_driver_count ] = (trigger_driver_s *)calloc(1, sizeof(trigger_driver_s));
...
// Buzzer Driver ------------------------------------------------------------------------------------
    ...
    action_drivers[ action_driver_count ] = (action_driver_s *)calloc( 1, sizeof( action_driver_s ) );
